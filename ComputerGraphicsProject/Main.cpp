...
HRESULT InitWindow(HINSTANCE hInstance, int nCmdShow)
{
	...
	g_hWnd = CreateWindow(L"AppWindowClass", L"Direct3D 11 Application",
		WS_OVERLAPPEDWINDOW,
...
{
	// Update our time
	pAnnotation->BeginEvent(L"Change cube angle");

	std::string textureName = "test_resource";
	ID3D11Resource* pResource;
	g_pRenderTargetView->GetResource(&pResource);

	pResource->SetPrivateData(WKPDID_D3DDebugObjectName,
		textureName.size(), textureName.c_str());
	...
	// Renders a triangle
	//
	pAnnotation->BeginEvent(L"Render cube");
